...
//    }
    ...
    "return 200 OK on active enrolments given no debit" in {
      ...
      val totalsUrl = s"$baseUrl/${empRef.taxOfficeNumber}/${empRef.taxOfficeReference}/total/"
//      val totalsUrl = s"$baseUrl/"

      println("***********************************")
      println(app.injector.instanceOf[Router])
      println(totalsUrl)
      println("***********************************")

      given()
        .client(empRef).isAuthorized
        .epayeTotalsReturns(owed = 0)
        .when()
